...
- [DONE] **Skip Intro & Render Pass Mask Fix (`MCLA_SkipIntroRenderPassMask`)**: Hooked `0x822C2F08` (`SkipIntro`) and `0x821315E4` (`MCLA_SkipIntroRenderPassMask`) to skip legal movies and mask out bit 24 (`0xFEFFFFFFu`), preventing the engine from submitting an uninitialized extra render pass that corrupted Downtown shaders.
